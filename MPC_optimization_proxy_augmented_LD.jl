...
global A2=zeros(T,J)
global A4=zeros(T,J)
global A5=zeros(T+1,J)

## constraint matrices based on the MILP formulation of this case study

for i=1:T
    for j=1:J
        global A[i,i]=-100/tou
        global A[i,i+1]=100/tou     ## -Et/tou + Et+1/tou -Pteng <= -Ptdes
        global A[i,T+1+i]=-1  

        global A1[i,1*(T)+1+i]=1   # Peng<=z Pmax
        global A1[i,2*(T)+1+i]=-Pmax
...
for j=1:size(arrk,1)
    ...
    #=global λ1=ones(31)*0.001
    global λ2=ones(30)*0.001
    global λ3=ones(30)*0.001
    global γ=ones(30)*0.001=#
#=
    @constraint(Q2,(A5*y_train'[i,:].-Emax).<=pen[i,1:T+1])
    @constraint(Q2,pen[i,1:T+1].>=0)

    @constraint(Q2,(A*y_train'[i,:].+Ptdes1[i,:]).<=pen[i,T+2+V:2I+V+1])
    @constraint(Q2,pen[i,T+2+V:2I+V+1].>=0)

    @constraint(Q2,(A1*y_train'[i,:]).<=pen[i,2I+2+V:3I+V+1])
    @constraint(Q2,pen[i,2I+2+V:3I+V+1].>=0)

    @constraint(Q2,(A4*y_train'[i,:].-Eo1[i]).<=pen1[i,:])
    @constraint(Q2,-(A4*y_train'[i,:].-Eo1[i]).<=pen1[i,:])=#

    #Emax=100/100
#
    loss(x,y)=mse(model(x),y)+sum(λ1'*sum(max.(0,A5*model(x).-Emax), dims=2)+λ2'*sum(max.(0,A1*model(x)), dims=2)+λ3'*sum(max.(0,A*model(x).+Ptdes1[1:k,:]'), dims=2)+γ'*sum(abs.((A4*model(x).-Eo1[1]/100)), dims=2))
    #loss2(x,y)=3
...
#
    for epoch in 1:epochs
        ...
        ytr=model(x_train)
        if (epoch<=300)
            global λ1=λ1.+0.1*(sum(max.(0,A5*ytr.-Emax), dims=2))/(k)  # update at each epoch
            global λ2=λ2.+0.001*(sum(max.(0,A1*ytr), dims=2))/(k)
            global λ3=λ3.+0.001*(sum(max.(0,A*ytr.+Ptdes1[1:k,:]'), dims=2))/(k)
            
            global γ=γ.+0.001*sum(abs.((A4*ytr.-Eo1[1]/95)), dims=2)/(k)
        end
